...
export default class TrafficLights {
  ...
  private getObjectForDirection(dir: string): three.Object3D {
    console.log(`DEBUG: Received turn direction: "${dir}"`);  // Debugging
    ...
    if (dir === Directions.LEFT || dir === Directions.PARTIAL_LEFT) {
      ...
      console.warn(`WARNING: Unrecognized turn direction "${dir}". Defaulting to straight.`);
      return this.arrows.straight;  // Default instead of throwing an error.
    }
  }  
}
